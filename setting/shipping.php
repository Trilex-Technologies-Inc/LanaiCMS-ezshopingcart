<?php
	...
	<?=_EZSHOP_SHIPPING_SETTING_INSTRUCTION; ?><br/><br/>
    
	<img src="theme/<?=$cfg['theme']; ?>/images/new.gif" border="0" align="absmiddle"/>
	<a href="<?=$_SERVER['PHP_SELF']?>?modname=<?=$module_name?>&mf=shippingnew" >
	<?=_NEW; ?></a>&nbsp;&nbsp;
    ...
    <!--
		function chk_mdelete() {
			if (confirm("<?=_DELETE_QUESTION; ?>")){
				document.form.ac.value="mshippingdelete";
				document.form.submit();
			}
		}
		function chk_mactive() {
			document.form.ac.value="mshippingactive";
			document.form.submit();
		}
	//-->
    </script>
<?
	$ezshop->getShippingList();
